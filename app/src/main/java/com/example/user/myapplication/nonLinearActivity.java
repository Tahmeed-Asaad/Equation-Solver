...
public class nonLinearActivity extends AppCompatActivity implements OnClickListener {
    ...
    public void graph(){
        ...
        try{
            ...
            rss=new BAIRSTOW(coeff).findroot11();
            Collections.sort(rss);
            //congratsMessage.setText("RSS=="+rss.elementAt(0)+"Rss=="+rss.elementAt(1));

            int s=rss.size();
            for(int i=0;i<s;i++){
                double xx=rss.elementAt(i);
                double yy=Horner(xx);
                serr.appendData(new DataPoint(xx, yy), true, 200);
            }
        }
        catch (Exception e){
            Toast.makeText(this,"The equation  has a complex root so the roots are not marked in the graph ", Toast.LENGTH_LONG).show();
        }
     ...
     //   g.getViewport().setMinY(0);
        ...
        g.getViewport().setMinX(-50);
        g.getViewport().setMaxX(170);
